...
- **Campaign** (`linkedin/models.py`) — `name` (unique), `users` (M2M to User for membership), `product_pitch`, `campaign_objective`, `booking_link`, `is_freemium` (bool), `action_fraction` (float), `seed_public_ids` (JSONField).
