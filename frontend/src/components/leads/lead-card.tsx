...
import { AddToCampaignModal } from '../modals/add-to-campaign-modal'
...
export function LeadCard({ lead, onView, onEdit, onDisqualify, onReScrape, onAddToCampaign }: LeadCardProps) {
  ...
  const handleAddToCampaign = (campaignId: string) => {
    if (onAddToCampaign) {
      onAddToCampaign(lead, campaignId)
    }
    setShowAddToCampaign(false)
  }

  const lastNotes = lead.lastNotes || []
  const hasNotes = (lead.notesCount || 0) > 0 || lastNotes.length > 0
  ...
  return (
    <>
        ...
        <CardContent className="pt-0">
          ...
          {hasNotes && (
            <div className="mb-3">
              <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                ...
                <span className="text-xs">({lead.notesCount || lastNotes.length})</span>
              </div>
              <div className="space-y-2">
                {lastNotes.slice(0, expandedNotes ? undefined : 2).map((note, idx) => (
                  <div key={idx} className="p-2 bg-muted/30 rounded-md text-xs whitespace-pre-wrap">
                    {note.content}
                  </div>
                ))}
                {lastNotes.length > 2 && !expandedNotes && (
                  <button
                    onClick={() => setExpandedNotes(true)}
                    className="text-xs text-primary hover:underline ml-1"
                  >
                    View all {lead.notesCount || lastNotes.length} notes
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button
              ...
              disabled={onAddToCampaign === undefined}
              title={onAddToCampaign ? "Add to Campaign" : "No campaign callback provided"}
            >
      ...
      <AddToCampaignModal
        ...
        leadId={lead.id}
        leadName={lead.name}
        onSuccess={onAddToCampaign ? () => onAddToCampaign(lead, '') : undefined}
      />
    </>
  )
}
