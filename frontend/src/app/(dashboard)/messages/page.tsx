...
function calculateCampaignStats(msgs: Message[]) {
  const stats: Record<
    string,
    { sent: number; received: number; responseRate: number }
  > = {};
  ...
  Object.keys(stats).forEach((key) => {
    ...
    stats[key].responseRate =
      total > 0 ? Math.round((stats[key].received / total) * 100) : 0;
  });
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Object.keys(stats.campaigns).length}
            </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className={getStatusColor(message.isOutgoing)}
                            >
                  ...
                  messages
                    ...
                    .map((msg) => {
                      const hasResponse = messages.some(
                        (m2) => m2.dealId === msg.dealId && !m2.isOutgoing,
                      );
              ...
              <div className="space-y-3">
                {Array.from(
                  ...
                  const outgoing = messages.find(
                    (m) => m.dealId === dealId && m.isOutgoing,
                  );
                  return outgoing ? (
                    <div
                      key={dealId}
                        ...
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-muted-foreground flex-shrink-0">
                            {outgoing.creationDate
                              ? formatDistanceToNow(
                                  new Date(outgoing.creationDate),
                                  { addSuffix: true },
                                )
                              : "Unknown date"}
                          </span>
      ...
      {selectedMessage && (
        ...
        >
          <DialogContent
            className={`${zincDialogContentClassName} flex max-h-[80vh] max-w-2xl flex-col overflow-hidden p-0`}
          >
            <DialogHeader
              className={`${zincDialogHeaderClassName} px-6 pt-6 sm:px-8 sm:pt-8`}
            >
              <DialogTitle>
                Conversation with {selectedMessage.recipientName || "Lead"}
              </DialogTitle>
              <DialogDescription>
            ...
            <div className="flex-1 overflow-hidden px-6 pb-6 sm:px-8 sm:pb-8">
              <div className="flex flex-col h-full">
                <Card className="flex flex-1 flex-col border-zinc-800 bg-zinc-900/40">
                  <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
                    {modalLoading ? (
                      ...
                      <div className="text-center py-8">
                        <Icons.MessageSquare className="mx-auto mb-4 h-12 w-12 text-zinc-400" />
