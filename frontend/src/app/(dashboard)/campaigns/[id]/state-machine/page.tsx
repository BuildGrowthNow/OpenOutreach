...
export default function StateMachinePage() {
  ...
  const [stateMachine, setStateMachine] =
    useState<StateMachineViewModel | null>(null);
  ...
  const [simulationStartState, setSimulationStartState] = useState<
    string | undefined
  >(undefined);
  ...
  // When the simulation dialog opens, initialize the start state from the current state machine
  useEffect(() => {
    if (simulationDialogOpen) {
      setSimulationStartState(stateMachine?.startState || "new");
    }
  }, [simulationDialogOpen, stateMachine]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (stateMachineResponse.data) {
        setStateMachine(
          stateMachineResponse.data as unknown as StateMachineViewModel,
        );
      } else if (
        stateMachineResponse.error &&
        !stateMachineResponse.error.includes("not found")
      ) {
  ...
  const handleSimulate = async () => {
    ...
    try {
      ...
      const response = await simulateStateMachineExecution(campaignId, {
        ...
        startState: simulationStartState || stateMachine?.startState || "new",
        maxSteps: 10,
      });
  ...
  const defaultTransitions: Array<
  ...
  > = [
    {
      from: "new",
      to: "contacted",
      trigger: "SEND_INITIAL_MESSAGE",
      description: "Send initial contact message",
      conditions: [],
      actions: [],
    },
    {
      ...
      description: "Lead responds positively",
      conditions: [],
      actions: [],
    },
    {
    ...
    },
    {
      ...
      description: "Book and complete meeting",
      conditions: [],
      actions: [],
    },
    {
      ...
      description: "Lead decides not to proceed",
      conditions: [],
      actions: [],
    },
    {
      from: "new",
      to: "disqualified",
      trigger: "INVALID_LEAD",
      description: "Lead information invalid",
      conditions: [],
      actions: [],
    },
  ];
  ...
  const hasStateMachine =
    stateMachine && stateMachine.states && stateMachine.transitions;
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setSimulationStartState(e.target.value)}
                    >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold">
                {hasStateMachine
                  ? ((stateMachine as StateMachineViewModel).states?.length ??
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">
                {hasStateMachine
                  ? ((stateMachine as StateMachineViewModel).transitions
                      ?.length ?? defaultTransitions.length)
                  : defaultTransitions.length}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">
                {hasStateMachine ? stateMachine.executionCount || 0 : 0}
              </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <>
                    ...
                    <div className="text-center">
                      <div className="inline-block p-6 bg-gray-100 dark:bg-gray-800 rounded-xl">
                        ...
                        <p className="text-sm text-muted-foreground mb-4">
                          Interactive workflow visualization for your campaign
                          automation
                        </p>
                        <div className="flex flex-wrap gap-3 justify-center">
                          {(stateMachine.states || defaultStates).map(
                            (state) => (
                              <div
                                key={state.id}
                                className={cn(
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Button
                  variant="outline"
                  onClick={() =>
                    router.push(`/state-machine?campaignId=${campaignId}`)
                  }
                >
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      // Try to export the in-memory state machine or fetch it
                      const machine =
                        stateMachine ||
                        (await getStateMachine(campaignId)).data ||
                        null;
                      const blob = new Blob(
                      ...
                      a.href = url;
                      a.download = `campaign-${campaignId}-state-machine.json`;
              ...
              <CardContent className="py-12">
                <div className="text-center max-w-md mx-auto">
                  ...
                  <p className="text-sm text-muted-foreground mb-6">
                    Create a state machine to automate lead progression through
                    your campaign. States represent lead statuses, transitions
                    represent actions that move leads between states.
                  </p>
                  <div className="flex gap-3 justify-center">
                    <Button
                      onClick={async () => {
                        try {
                          setLoading(true);
                          // Build a simple default graph compatible with the global editor
                          const nodes = (defaultStates || []).map((s, i) => ({
                            id: s.id,
                            type: "state",
                            name: s.name,
                          ...
                          const transitions = (defaultTransitions || []).map(
                            (t, i) => ({
                              id: `t-${i}-${Date.now()}`,
                              source_node: t.from,
                              target_node: t.to,
                          ...
                          const response = await updateStateMachine(
                            campaignId,
                            {
                              name: `${campaign?.name || "Campaign"} Default State Machine`,
                              description:
                                "Automatically generated default state machine",
                              graph_data: { nodes, transitions },
                            },
                          );

                          if (response && response.data) {
                            // Open the full editor for this campaign
                            router.push(
                              `/state-machine?campaignId=${campaignId}`,
                            );
                          } else {
                            setError(
                              response.error ||
                          ...
                          setError(
                            "An error occurred while creating the default state machine",
                          );
                          console.error(err);
                        } finally {
                          setLoading(false);
                    ...
                    <Button
                      variant="outline"
                      onClick={async () => {
                        // Use the same default template flow for now
                        try {
                          setLoading(true);
                          const nodes = (defaultStates || []).map((s, i) => ({
                            id: s.id,
                          ...
                          const response = await updateStateMachine(
                            campaignId,
                            {
                              ...
                              description: "Template state machine",
                              graph_data: { nodes, transitions },
                            },
                          );
                          ...
                          if (response && response.data) {
                            ...
                            setError(
                              response.error || "Failed to apply template",
                            );
                          }
                        } catch (err) {
                          setError(
        ...
        <TabsContent value="states" className="space-y-6">
                ...
                {(stateMachine?.states || defaultStates).map((state) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={cn("p-2 rounded", getStateColor(state.id))}
                      >
                        {getStateIcon(state.id)}
                      </div>
                      <div>
          ...
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">
              States define the possible statuses a lead can have in your
              campaign workflow
            </p>
            <Button
              onClick={() =>
        ...
        <TabsContent value="transitions" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define how leads move between states based on events and
                conditions
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {(stateMachine?.transitions || defaultTransitions).map(
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      className="w-full mt-1 p-2 border rounded-md"
                      rows={2}
                      placeholder="Describe what triggers this transition..."
                    />
                  </div>
                </div>
              </div>
