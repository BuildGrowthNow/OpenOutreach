...
class StateMachineEngine:
    ...
    def evaluate_condition(
        self, node: StateNode, deal: Deal, last_message: Optional[Dict] = None
    ) -> bool:
        ...
        if condition_type == "always":
            ...
            return (
                deal.messages.count() >= count if hasattr(deal, "messages") else False
            )

        return True

    def next_node(
        self, current_node: StateNode, deal: Deal, last_message: Optional[Dict] = None
    ) -> Optional[StateNode]:
    ...
    def execute_step(
        self, state_machine: CampaignState, session=None
    ) -> Tuple[bool, str]:
    ...
    def _execute_node(self, node: StateNode, deal: Deal, session=None) -> Dict:
        ...
        if node.node_type == StateNode.TYPE_WAIT:
            ...
            linkedin_profile = (
                ...
                if session and hasattr(session, "linkedin_profile")
                else None
            )
            ...
            # Send message (creates outgoing message in database)
    ...
    def validate_graph(self) -> Tuple[bool, List[Dict]]:
            ...
            errors.append(
                {
                    ...
                    "message": f"Multiple start nodes defined: {start_nodes.count()}",
                }
            )
        ...
        for start_node in start_nodes:
            if has_cycle(start_node.id):
                errors.append(
                    {"type": "cycles", "message": "Cycle detected in state graph"}
                )
                break
        ...
        for node in nodes:
            if node.id not in reachable:
                warnings.append(
                    {
                        "type": "unreachable",
                        "message": f"Node '{node.name}' (ID: {node.id}) is unreachable",
                    }
                )
    ...
    def _execute_link_node(self, node: StateNode, deal: Deal, session=None) -> Dict:
        ...
        try:
            ...
            if link_id:
                ...
                tracked_link, created = TrackedLink.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "utm_campaign": (
                            deal.campaign.name if deal.campaign else link_type
                        ),
                    },
                )
            ...
            try:
                ...
                # Additional clicks are recorded when users visit the link via link_redirect view
    ...
    def simulate_execution(
        self, deal: Deal, start_node_id: Optional[int] = None
    ) -> Dict:
        ...
        simulation["final_status"] = (
            "completed"
            if current_node and current_node.node_type == StateNode.TYPE_END
            else "incomplete"
        )
