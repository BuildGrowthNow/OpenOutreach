...
def generate_lead_persona(session, deal: "Deal") -> LeadPersonaOutput | None:
    ...
    """
    from openoutreach.mongodb.models import Lead, Campaign
    import jinja2

    # Fetch full profile from LinkedIn
    lead = Lead.get(deal.lead_id)
    if not lead:
        logger.warning(f"Could not fetch lead for deal {deal.pk}")
        return None
    ...
    if not campaign:
        logger.warning(f"Could not fetch campaign for deal {deal.pk}")
        return None
    ...
    if result.output is None:
        lead_id = deal.lead.public_identifier if deal.lead else deal.lead_id
        logger.error(f"LLM returned unparseable persona for {lead_id}")
        return None
...
def save_persona(deal: "Deal", persona_output: LeadPersonaOutput) -> None:
    ...
    """
    from openoutreach.mongodb.models import LeadPersona, Lead
    from openoutreach.mongodb.connection import get_mongodb_collection

    collection = get_mongodb_collection("lead_personas")
    if collection is None:
        logger.error("MongoDB collection 'lead_personas' not available")
        return
    ...
    if existing_doc:
        ...
        persona = LeadPersona.from_dict(existing_doc)
    ...
    lead = Lead.get(deal.lead_id)
    lead_identifier = lead.public_identifier if lead else deal.lead_id
    logger.info(
        f"Persona v{persona.version} saved for {lead_identifier} "
...
def get_lead_persona(deal: "Deal") -> "LeadPersona | None":
    ...
    """
    from openoutreach.mongodb.models import LeadPersona
    from openoutreach.mongodb.connection import get_mongodb_collection
    ...
    # Find most recent version for this lead/campaign
    doc = collection.find_one(
        {"lead_id": deal.lead_id, "campaign_id": deal.campaign_id},
        sort=[("version", -1)]
    )
    return LeadPersona.from_dict(doc) if doc else None
