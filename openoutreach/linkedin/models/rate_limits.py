...
class SmartRateLimitContext(models.Model):
    ...
    linkedin_profile = models.OneToOneField(
        "LinkedInProfile", on_delete=models.CASCADE, related_name="rate_limit_context"
    )
    ...
    detectability_score = models.PositiveIntegerField(
        default=50
    )  # 0-100, 100 = detected
    ...
    campaign_context = models.JSONField(
        default=dict
    )  # {campaign_id: {phase, lead_count, etc}}
    ...
    def get_effective_limit(self, action_type: str, campaign=None) -> int:
        ...
        if campaign and str(campaign.id) in self.campaign_context:
            campaign_limit = self.campaign_context[str(campaign.id)].get(
                "limit", float("inf")
            )
    ...
    def update_detectability(self, score_delta: int):
        ...
        self.detectability_score = max(
            0, min(100, self.detectability_score + score_delta)
        )
...
class RateLimitWarning(models.Model):
    ...
    linkedin_profile = models.ForeignKey(
        "LinkedInProfile", on_delete=models.CASCADE, related_name="rate_limit_warnings"
    )
