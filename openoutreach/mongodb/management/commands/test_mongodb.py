...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if check_mongodb_connection():
            ...
            self.stdout.write(
                self.style.WARNING("   [INFO] MongoDB not enabled or not connected")
            )
        ...
        if db:
            ...
            self.stdout.write(
                f"   Campaign performance pipeline has {len(pipeline)} stages"
            )
        ...
        self.stdout.write("=" * 60)
        self.stdout.write(
            self.style.SUCCESS("MongoDB integration test completed successfully")
        )
        self.stdout.write("\nNext steps:")
        self.stdout.write("1. Add 'djongo' to DATABASES settings in settings.py")
        self.stdout.write(
            "2. Run 'python manage.py migrate' to create MongoDB collections"
        )
        self.stdout.write(
            "3. Use 'python manage.py test_mongodb --create-indexes' to create indexes"
        )
        self.stdout.write("\nFor production:")
