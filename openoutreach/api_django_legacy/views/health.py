...
try:
    ...
    check_mongodb_connection = None  # type: ignore
...
def _calculate_database_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        success_rate = round(
            (successful_actions / total_actions * 100) if total_actions > 0 else 100, 1
        )
        ...
        try:
            ...
            if total_timing_actions > 0:
                ...
                try:
                    avg_latency = (
                        successful_with_timing.aggregate(avg_duration=models.Avg("duration_ms"))[
                            "avg_duration"
                        ]
                        or 0
                    )
...
def _check_services() -> Dict[str, Any]:
    ...
    services: Dict[str, str] = {
        ...
        "mongodb": (
            "operational"
            if (MONGODB_AVAILABLE and _check_mongodb().get("connected", False))
            else "degraded"
        ),
