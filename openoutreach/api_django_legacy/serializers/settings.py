...
class RateLimitsSerializer(serializers.Serializer):
    ...
    # Daily limits
    daily_connection_limit = serializers.IntegerField(
        default=50, min_value=1, max_value=1000
    )
    daily_follow_up_limit = serializers.IntegerField(
        default=20, min_value=1, max_value=500
    )

    # Global limits
    global_rate_limit_seconds = serializers.IntegerField(
        default=60, min_value=10, max_value=3600
    )
    global_rate_limit_requests = serializers.IntegerField(
        default=100, min_value=10, max_value=1000
    )

    # Safety limits
    safety_connection_limit = serializers.IntegerField(
        default=30, min_value=1, max_value=500
    )
    safety_follow_up_limit = serializers.IntegerField(
        default=15, min_value=1, max_value=200
    )

    # Cooldown settings
    min_cooldown_seconds = serializers.IntegerField(
        default=60, min_value=10, max_value=3600
    )
    max_cooldown_seconds = serializers.IntegerField(
...
class SystemSettingsSerializer(serializers.Serializer):
    ...
    default_theme = serializers.ChoiceField(
        choices=["light", "dark", "system"], default="dark"
    )
    ...
    api_rate_window_seconds = serializers.IntegerField(
    ...
    smtp_port = serializers.IntegerField(
        required=False, default=587, min_value=1, max_value=65535
    )
    smtp_use_tls = serializers.BooleanField(default=True)

    # LinkedIn Settings
    linkedin_timeout_seconds = serializers.IntegerField(
        default=30, min_value=5, max_value=120
    )
    linkedin_retry_count = serializers.IntegerField(
