...
class LinkedInCredentialsRotationView(APIView):
    ...
    def post(self, request, pk=None):
        ...
        data = request.data
        new_email = data.get('email', cred.get_email() if hasattr(cred, 'get_email') else None)
        new_password = data.get('password', cred.get_password() if hasattr(cred, 'get_password') else None)
