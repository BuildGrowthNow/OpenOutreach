...
class LeadListView(APIView):
    ...
    def get(self, request):
        ...
        all_deals = Deal.objects.filter(lead__in=leads).select_related(
            "campaign", "lead"
        )
            ...
            leads_data.append(
                {
                    ...
                    "campaignName": (
                        deal.campaign.name if deal and deal.campaign else None
                    ),
                    "creationDate": (
                        lead.creation_date.isoformat() if lead.creation_date else None
                    ),
                    "updateDate": (
                        lead.update_date.isoformat() if lead.update_date else None
                    ),
    ...
    def post(self, request):
        ...
        if not public_identifier:
            return Response(
                {"success": False, "error": "Public identifier is required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class LeadDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        last_message = (
            Message.objects.filter(deal__lead=lead).order_by("-created_at").first()
        )
    ...
    def patch(self, request, pk):
        ...
        if (
            ...
            and public_identifier != lead.public_identifier
        ):
...
class LeadMessagesView(APIView):
    ...
    def get(self, request, pk):
        ...
        messages = (
            Message.objects.filter(deal__lead=lead)
            .select_related("deal")
            .order_by("-created_at")
        )
            ...
            messages_data.append(
                {
                    ...
                    "dealUrn": (
                        msg.deal.lead.public_identifier
                        if msg.deal and msg.deal.lead
                        else ""
                    ),
                    "content": msg.content,
                    ...
                    "recipientUrl": (
                        msg.deal.lead.linkedin_url if msg.deal and msg.deal.lead else ""
                    ),
                }
            )
        ...
        return Response(
            {
                ...
                "message": {
                    ...
                    "dealUrn": (
                        message.deal.lead.public_identifier
                        if message.deal and message.deal.lead
                        else ""
                    ),
                    ...
                    "recipientUrl": (
                        message.deal.lead.linkedin_url
                        if message.deal and message.deal.lead
                        else ""
                    ),
                },
...
class LeadNotesView(APIView):
    ...
    def get(self, request, pk):
        ...
        notes = (
            Note.objects.filter(deal__lead=lead)
