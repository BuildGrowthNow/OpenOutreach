...
openoutreach/
  ...
  contacts/          # central contacts-store client (service.py only, no models)
```
...
- `collect_from_wizard()` — interactive questionary wizard (needs TTY), only asks for `missing_keys()`. Backed by the vendored `onboarding_wizard.py` (step engine) + `onboarding_prompts.py` (`SELF_HOSTED_QUESTIONS`) — no external `openoutreach` dependency. When the LLM fields are collected, `_verify_llm_answers` does a live `verify_llm_credentials` ping and re-asks the three LLM questions until the key/model actually work (catches a bad/wrong-provider/non-ASCII key at onboarding instead of at the first task).
...
Persistent queue backed by `Task` model. Worker loop in `daemon.py`: `seconds_until_active()` guard pauses outside the daily active-hours window (single contiguous window, no weekend skip) → claim next due task (`claim_next`, **email-first then oldest-due** — see `TaskQuerySet.pending`) → set campaign on session → RUNNING → dispatch via `_HANDLERS` dict → COMPLETED/FAILED. Failures captured by `failure_diagnostics()` context manager. The daemon runs `reconcile(session)` once before the loop, so a fresh start recovers crash-stale RUNNING tasks and flushes ready emails before serving — first thing it does is send any email it can.
...
3. **`reconcile(session)`** — Recovers stale RUNNING tasks, then per campaign runs the window planners and the email flush. Daemon calls it once on startup (before the loop) and whenever the queue has no ready task. Combined with email-first claim ordering, the startup call is what makes the daemon send any ready email before touching the LinkedIn backlog.
...
Apps in `INSTALLED_APPS`, all nested under the `openoutreach/` package (see Project Layout):
...
- **`emails`** — Email channel (Layer 1 of the email-first pivot). `bettercontact.py` resolves a work email for a qualified lead on demand — the *paid* finder, distinct from the free hub lookup (`contacts.resolve`): `resolve_email()` is the public facade (SiteConfig-gated), `find_email()` the submit→poll client over the BetterContact API, `is_configured()` reports whether a key is set; `BetterContactQuery`/`BetterContactResult`/`BetterContactUnavailable` are its types. **`Mailbox`** model (one SMTP inbox; module-level `has_mailbox()` is the "email is a viable channel" gate — ≥1 box; host/port default to IceMail's Google boxes `smtp.gmail.com:587`); **`icemail.py`** `parse_mailboxes()` reads the `Email`/`Password` columns from a pasted *Export Mailboxes* block; **`smtp.py`** `verify_auth()` is the auth-only login check (no test send during warmup); **`nudge.py`** is the per-launch setup prompt (`email_state()` machine + GLF copy + `import_mailboxes()` = parse→store→auth-check). **`sender.py`** `send_email(mailbox, to, subject, body, *, bcc=None, ...)` sends one message over the box's SMTP+STARTTLS creds and returns the Message-ID (no error handling by design — a failed send fails its task and is retried). `handle_email` passes `bcc=session.linkedin_profile.linkedin_username` so the operator's own LinkedIn login email is blind-copied on every send (a private record to follow up threads in person); sends go out from throwaway IceMail boxes, so the operator's address is the BCC, not the From. `send_message` strips the Bcc header before transmission, so the recipient never sees it. **`tasks/send.py`** `handle_email` is the single-shot EMAIL task: pick the least-loaded under-cap `Mailbox` + the oldest READY_TO_EMAIL deal (`core.db.deals.get_emailable_deals`), compose via `core/agents/email_opener.py`, send, then `_record_sent_email` writes the email fields **and** `state=EMAILED` in one `deal.save()` (send record + state on the same row → no double-send window). Per-box daily-cap pacing lives on the `Mailbox` manager (`least_loaded_under_cap()`, `remaining_today()`, instance `sent_today()`/`headroom_today()`, keyed on `Deal.email_sent_at`). Layer 1 is outbound-only and never re-emails — follow-ups/replies are the hosted Layer-2 backend's job, reconstructed from the mailbox.
...
- **`contacts`** — Central contacts-store client (`service.py`, no models) — "the hub" (`hub.openoutreach.app`); logs under the `hub:` prefix. `resolve(lead)` is the free hub lookup, tried before the paid BetterContact finder; `contribute(session, lead, emails, origin)` at the two contact moments (non-EU only, registers on first use). Both enrichment lookups sit behind `emails.models.has_mailbox()` in `_resolve_email` — no mailbox, no resolution. See Key Modules.
...
- **SiteConfig** (`core/models.py`) — Singleton (pk=1). `ai_model` (a pydantic-ai `provider:model` identifier — the provider lives inside the model string; valid providers: openai/anthropic/google/groq/mistral/cohere/openai_compatible), `llm_api_key`, `llm_api_base` (only for `openai_compatible:*`), `bettercontact_api_key` (BetterContact email-finder key; blank disables enrichment — see `emails/bettercontact.py`), `contacts_api_token`/`contacts_api_url` (central contacts store — token earned on first contribution, blank URL → the default hub; see `contacts/service.py`). Accessed via `SiteConfig.load()`; `core/llm.py:get_llm_model()` is the single factory that turns it into a `pydantic_ai.models.Model` (`split_model_id` parses the provider out of `ai_model` and dispatches to the per-provider builder).
...
- **LinkedInProfile** (`linkedin/models.py`) — 1:1 with User. `self_lead` FK to Lead (nullable, set on first self-profile discovery). Credentials, rate limits (`connect_daily_limit`, `follow_up_daily_limit` — daily-only; LinkedIn's own weekly ceiling surfaces at the handler boundary via `ReachedConnectionLimit`). `subscribe_newsletter` + `contribute_to_hub` consent flags; `subscribe_newsletter` defaults on and is asked at onboarding (`setup/geo.py` force-enables it outside the broad newsletter set), while `contribute_to_hub` is **not asked** — `setup/geo.py:apply_gdpr_contribution_override` sets it from the operator's LinkedIn country at first daemon run: on outside the EEA/UK/CH collection set, off inside it (or unknown location). `contribute_to_hub` gates the entire contacts-store give-back (see `contacts/service.py`). Methods: `can_execute`/`record_action`/`mark_exhausted`. In-memory `_exhausted` dict for daily rate limit caching.
...
- **`core/db/deals.py`** — Deal/state ops, `set_profile_state()` (also fires `capture_and_contribute()` on the CONNECTED transition; the hub give-back rides the `contact_info` null→non-null transition, so a repeat follow-up visit no-ops instead of re-sending), `increment_connect_attempts()`, `create_freemium_deal()`.
...
- **`core/llm.py`** — `get_llm_model()` factory + `run_agent_sync(coro)` sync boundary. `build_llm_model(ai_model, api_key, api_base)` is the shared builder (from explicit creds); `get_llm_model()` calls it with `SiteConfig`. `verify_llm_credentials(...)` does a one-shot live ping (used by onboarding to validate a key before saving), retrying transient failures via `tenacity` and returning a human-readable error string otherwise. `get_llm_model()` reads `SiteConfig` and dispatches via per-provider builders (OpenAI / Anthropic / Google / Groq / Mistral / Cohere / openai_compatible) to the right `pydantic_ai.models.Model`. Call sites build `Agent(get_llm_model(), ...)` and invoke `run_agent_sync(agent.run(prompt))` — never `Agent.run_sync`, whose anyio portal leaves the caller's thread running-loop slot populated and poisons subsequent sync Playwright calls (`"using Playwright Sync API inside the asyncio loop"`). `run_agent_sync` drives the coroutine to completion on a short-lived worker thread with its own event loop; per-thread asyncio slots are independent, so the caller's thread stays clean regardless of what anyio / pytest-anyio / Jupyter / etc. did to it.
...
- **`linkedin/setup/geo.py`** — country-code jurisdiction lines: `is_gdpr_protected()`/`apply_gdpr_newsletter_override()` (broad email-opt-in set, drives newsletter) and `is_eea_located()`/`EEA_UK_CH` (narrow EEA/UK/CH collection-regime set; the client geo-pre-filter for contacts-store contribution — the server re-gates authoritatively).
- **`contacts/service.py`** — central contacts-store client: `resolve(lead)` (free read-back before the paid finder; `/resolve` returns an `emails[]` array — one address today, the full deduped set once the dbt mart lands — and the client takes the first for the singular `Lead.api_email`) and `contribute(session, lead, emails, origin)` (give-back at the two contact moments, non-EU only, registering on first use; `origin` is `ORIGIN_BETTERCONTACT` or `ORIGIN_PROFILE_INFO`). Best-effort; an outage or missing token degrades to a no-op. Reads `SiteConfig.contacts_api_token`/`contacts_api_url`; sends `{public_identifier, country_code, emails, origin}` (no embedding).
...
- **`settings.py`** — Django settings (SQLite at `data/db.sqlite3`). Apps: crm, chat, core, linkedin, emails, contacts.
...
- **`SiteConfig`** (DB singleton) — `ai_model` (required — a `provider:model` identifier, e.g. `anthropic:claude-sonnet-4-5-20250929`; bare `gpt-*`/`o1`/`o3`/`claude-*`/`gemini-*` is auto-prefixed, everything else needs the explicit prefix), `llm_api_key` (required), `llm_api_base` (required only for `openai_compatible:*`), `bettercontact_api_key` (optional — BetterContact email-finder key; blank disables enrichment), `contacts_api_token`/`contacts_api_url` (optional — central contacts store; token earned on first contribution, blank URL → the default hub). Editable via Django Admin.
- **`conf.py` schedule** — `ENABLE_ACTIVE_HOURS` (`False`), `ACTIVE_START_HOUR` (9), `ACTIVE_END_HOUR` (19), `ACTIVE_TIMEZONE` (`None` → resolved at runtime from the operator's LinkedIn profile country, once, after login; set an IANA name to override). The window timezone mimics the *operator's* waking rhythm, so it's keyed to the account, not the host: `AccountSession.active_timezone` returns the conf override if set, else `tz_country.timezone_for_country(profile country_code)` (pytz `country_timezones`), else `None`. `None` = no gating — and login itself is never gated (it runs before the loop and is what resolves the zone). The daemon and `scheduler` primitives (`seconds_until_active`, `working_seconds_in_window`, `poisson_slot_times`, `_working_intervals`) take the resolved zone as an argument; the daemon logs its provenance so an inferred (possibly wrong) zone is visible and overridable. Daemon sleeps outside this window. No weekend/rest-day handling — humans use LinkedIn 7 days a week.
