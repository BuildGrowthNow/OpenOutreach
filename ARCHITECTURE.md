...
1. **Per-type planners** — `plan_connect_window`, `plan_follow_up_window`, `plan_check_pending_window`. Each, when no PENDING task of its type exists for a campaign, computes the right slot count `n` for the next 24h and spaces tasks according to `SiteConfig.velocity` (actions per hour). When velocity >= 30 (aggressive mode: <= 2min spacing), tasks cluster into immediate bursts with 5-10 sec human rhythm. When velocity < 30, tasks spread uniformly via Poisson spacing across the working window. The scheduler reads active-hours config from `SiteConfig` DB fields (`enable_active_hours`, `active_start_hour`, `active_end_hour`, `active_timezone`, `active_days`) — deprecated `conf.py` constants are no longer consulted.
...
- **Campaign** (`core/models.py`) — `name` (unique), `users` (M2M to User), `product_pitch` (TextField — product description for agents), `campaign_objective`, `booking_link`, `icp_titles` (JSONField list of target job titles/roles), `follow_up_strategy` (TextField — agent follow-up instructions), `is_freemium`, `action_fraction`, `seed_public_ids` (JSONField).
