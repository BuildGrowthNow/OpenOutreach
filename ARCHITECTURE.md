...
- **`contacts`** — Central contacts-store client (`service.py`, no models). `resolve(lead)` before the paid finder; `contribute(session, lead, emails)` at the two contact moments (non-EU only, registers on first use). See Key Modules.
...
- **SiteConfig** (`core/models.py`) — Singleton (pk=1). `llm_provider` (TextChoices: openai/anthropic/google/groq/mistral/cohere/openai_compatible), `llm_api_key`, `ai_model`, `llm_api_base`, `finder_api_key` (BetterContact email-finder key; blank disables enrichment — see `emails/finder.py`), `contacts_api_token`/`contacts_api_url` (central contacts store — token earned on first contribution, blank URL → the default hub; see `contacts/service.py`). Accessed via `SiteConfig.load()`; `core/llm.py:get_llm_model()` is the single factory that turns it into a `pydantic_ai.models.Model`.
...
- **`contacts/service.py`** — central contacts-store client: `resolve(lead)` (free read-back before the paid finder; `/resolve` returns an `emails[]` array — one address today, the full deduped set once the dbt mart lands — and the client takes the first for the singular `Lead.api_email`) and `contribute(session, lead, emails)` (give-back at the two contact moments, non-EU only, registering on first use). Best-effort; an outage or missing token degrades to a no-op. Reads `SiteConfig.contacts_api_token`/`contacts_api_url`; sends `{public_identifier, country_code, emails}` (no embedding).
